...
public class AchievementDaoImpl implements AchievementDao {
        ...
        try {
            return jdbcTemplate.update(con -> {
                PreparedStatement ps = con.prepareStatement(
                        ...
                        "VALUES (?,?,CURRENT_TIMESTAMP) " + "ON CONFLICT (user_id, achievement_id) DO UPDATE " +
    ...
    public List<UserAchievement> getUserAchievements(String userId) {
        try {
            return jdbcTemplate.query(
                    "SELECT a.achievement_id, a.title, a.description, a.image_content, a.category_id, c.title AS category_title, ua.datetime_gained, ua.times_gained " +
                    "FROM achievements a INNER JOIN users_achievements ua ON a.achievement_id = ua.achievement_id " +
                    "LEFT JOIN categories c ON a.category_id = c.category_id " + "WHERE ua.user_id = uuid(?);",
    ...
    public List<UserAchievement> getUserAchievementsLastWeek(String userId) {
        return jdbcTemplate.query(
                "SELECT a.achievement_id, a.title, a.description, image_content, category_id, datetime_gained, times_gained" +
